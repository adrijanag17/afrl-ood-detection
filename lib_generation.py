...
def get_posterior(model, net_type, test_loader, magnitude, temperature, outf, out_flag):
    ...
    for data, _ in test_loader:
        ...
        gradient = (gradient.float() - 0.5) * 2
        if net_type == 'densenet':
            gradient.index_copy_(1, torch.LongTensor([0]).cpu(), gradient.index_select(1, torch.LongTensor([0]).cpu()) / (63.0/255.0))
            gradient.index_copy_(1, torch.LongTensor([1]).cpu(), gradient.index_select(1, torch.LongTensor([1]).cpu()) / (62.1/255.0))
            gradient.index_copy_(1, torch.LongTensor([2]).cpu(), gradient.index_select(1, torch.LongTensor([2]).cpu()) / (66.7/255.0))
        elif net_type == 'resnet':
            gradient.index_copy_(1, torch.LongTensor([0]).cpu(), gradient.index_select(1, torch.LongTensor([0]).cpu()) / (0.2023))
            gradient.index_copy_(1, torch.LongTensor([1]).cpu(), gradient.index_select(1, torch.LongTensor([1]).cpu()) / (0.1994))
            gradient.index_copy_(1, torch.LongTensor([2]).cpu(), gradient.index_select(1, torch.LongTensor([2]).cpu()) / (0.2010))
