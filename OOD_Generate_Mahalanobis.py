...
parser.add_argument('--net_type', required=True, help='resnet | densenet | vit')
...
def main():
    ...
    if args.dataset == 'cifar100':
        ...
        out_dist_list = ['cifar10', 'svhn']
    elif args.dataset == 'cifar10':
        out_dist_list = ['cifar100', 'svhn']
    else:
    ...
    if args.net_type == 'densenet':
        if args.dataset == 'svhn':
            ...
            model = torch.load(pre_trained_net, map_location = torch.device('cpu'))
    ...
    # add vit
        ...
        model = models.ViT16(num_c=args.num_classes)
        model.load_state_dict(torch.load(pre_trained_net, map_location= torch.device('cpu')))
        in_transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize((0.4914, 0.4822, 0.4465), (0.2023, 0.1994, 0.2010)),])
    model.cpu()
    ...
    model.eval()
    temp_x = torch.rand(2,3,32,32).cpu()
